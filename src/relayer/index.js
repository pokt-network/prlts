...
class Server {
    ...
    async submitRandomRelay(aats, pocket) {
        try {
            ...
            const session = await pocket.sessionManager.getCurrentSession(aat, payload.blockchain, pocket.configuration)
            ...
            if (payload.consensus_enabled === true) {
                ...
                responseOrError = await pocket.sendConsensusRelay(payload.data, payload.blockchain, aat, pocket.configuration, undefined, payload.method, payload.path, undefined)
            } else {
    ...
    async config() {
        ...
        this.logger.log("debug", "Successfully parsed dispatch url list", dispatchURLList)
        ...
        const clientPrivateKeyOrError = await keybase.exportAccount(clientAccount.addressHex, this.clientPassphrase)
        ...
        for (let index = 0; index < this.configuration.chains.length; index++) {
            ...
            for (let j = 0; j < chainConfig.application_private_keys.length; j++) {
                ...
                const appAccount = await keybase.importAccount(Buffer.from(appPK, "hex"), this.applicationPassphrase)
        ...
        this.logger.log("debug", `Succesfully configured ${this.configuration.parallelRelays} pocket instances`)
    }
    ...
    async start() {
        ...
        while (true) {
            const tasks = []
            for (let index = 0; index < this.configuration.parallelRelays; index++) {
                const pocketConfiguration = new PocketConfiguration(undefined, undefined, 5, this.configuration.relayTimeout, true, this.configuration.sessionBlockFrequency, this.configuration.blockTime, undefined)
